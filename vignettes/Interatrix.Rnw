...
The extension of the data file must be \textit{txt}, \textit{rda}, \textit{Rda}, \textit{rdata} or \textit{Rdata}.
...
When using the explicit functions, data must be loaded thanks to the command \texttt{read.table} (for \textit{txt} file) or \texttt{load} (for \texttt{R} binary file).
